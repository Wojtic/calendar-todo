...
initializePassport(passport, (email, callback) => {
...
app.post("/create_task", jsonParser, (req, res) => {
    if (!(req.body.name && req.body.date && req.body.owner)) {
        ...
        let queryString = `INSERT INTO tasks (task_name, task_description, task_date) VALUES ('${req.body.name}', '${req.body.description || ""}', '${req.body.date}')`;
        con.query(queryString, (err, response) => {
            ...
            con.query(queryString, (err, response) => {
                ...
                queryString = `INSERT INTO task_to_owner (task_id, ${req.body.owner.substring(0, 6) == "group-" ? "group_name" : "user_id"}) VALUES (${task_id}, ${req.body.owner.substring(0, 6) == "group-"
                    ? `'${req.body.owner}'`
                    : req.user.id})`;
                con.query(queryString, (err, response) => {
                    if (err)
